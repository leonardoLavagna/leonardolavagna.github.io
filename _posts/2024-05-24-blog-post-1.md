...
- Let $|\gamma\beta\rangle:=U(\beta_p)U_f(\gamma_p)\prod_{i=1}^{p-1}[U(\beta_{p-i})U_f(\gamma_{p-i})]\ket{\psi_0}$ be the evolved state and let $F_p$ be the expectation of $f$ with respect to this state obtained by repeated measurements, that is $F_p(\gamma,\beta):=\langle \gamma\beta| f(\sigma)\ |\ \gamma\beta\rangle$.
