...
&nbsp;&nbsp; A QUBO problem, like the MaxCut, can be mapped to an observable $H$ corresponding to the Hamiltonian of the [Ising model](https://en.wikipedia.org/wiki/Ising_model), of the form $$H(\sigma)=\sum_{i,j}w_{ij}(\mathbf{1}-\bm{\sigma}_z^{(i)}\sigma_z^{(j)}),$$
where the optimal solutions correspond to fundamental states of $H(\bm{\sigma})$, retrievable via [quantum adiabatic computations](https://en.wikipedia.org/wiki/Adiabatic_quantum_computation).
