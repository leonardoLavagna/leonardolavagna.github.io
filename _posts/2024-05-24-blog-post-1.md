...
&nbsp;&nbsp; we can think of a combinatorial optimization problem as a triple $(\mathcal{P},f,\textsc{Alg})$, where $\mathcal{P}$ is a finite set of $m$ (classical) logical propositions, $f:\{0,1\}^n\to \mathbb{Z}$ is the cost function associating a $n$-bit string $z$ to the number of satisfied clauses of the form
\begin{equation}
